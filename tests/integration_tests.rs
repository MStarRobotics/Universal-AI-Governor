...
async fn test_config_integration() {
    ...
    assert_eq!(Config::default().server.port, 8080);
    assert_eq!(Config::default().database.url, "sqlite://governor.db");
    assert!(!Config::default().security.enable_tpm);
    assert!(!Config::default().security.enable_hsm);
}
